...
class ErrorAnalysis:
    ...
    def get_metric_values(self, df: pd.DataFrame, keys) -> pd.DataFrame:
        """
        df.columns = [self.time_identifier] + self.keys_identifier +
                     [self.target_column_name, self.predicted_column_name]
    ...
    def cohort_plot(
        ...
        """
        df.columns = [self.time_identifier] + self.keys_identifier +
                     [self.target_column_name, self.predicted_column_name, lookahead, walk_name]
