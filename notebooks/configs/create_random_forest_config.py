...
# MAGIC %md
# MAGIC # Create Configuration File - Random Forest Example
# MAGIC 
# MAGIC This notebook demonstrates how you can create a parameterized configuration JSON file that will be later used for feature engineering, model training, and evaluation (See the **run_ml_experiment_mlflow** notebook). The intent is to create a reusable file containing the experimentation parameters that data scientists can leverage, with the goal of reducing overhead, promoting consistency in experimentation, and ensuring reproducibility.
# MAGIC 
...
manager = ConfigManager(path_to_config="./json/random_forest_config_small.json")
rf_config = manager.get()
pprint(rf_config)
...
# MAGIC     },
# MAGIC },
# MAGIC "feature_engineering": The feature engineering methods that will be applied to the dataset. See the `feature_engineering`
# MAGIC                        directory for a list of implemented feature engineering operations.
# MAGIC {
# MAGIC     "feature_horizon": The number of time steps (number of weeks, months, etc.) we would like to make predictions ahead
# MAGIC                        of time. For example, a value of 12 weeks would mean that we would like to predict the target
# MAGIC                        values 12 weeks in advance. This field is specifically used by autoregressive featurizers that
# MAGIC                        operate on the target column such as lag featurizer to prevent data leakage.
# MAGIC     "operations": {
# MAGIC         "one_hot_encoding": {
# MAGIC             "categorical_colnames": The list of categorical column names that one hot encoding features will be computed for.
# MAGIC         },
# MAGIC         "basic_time_based": {
# MAGIC             "feature_names": The list of feature names that will be computed based on time column.
# MAGIC         },
# MAGIC         "lags": {
# MAGIC             "colnames_to_lag": The list of column names to compute lags (i.e."weekly_rev").
# MAGIC             "lag_time_steps": The list of time steps to compute lags.
# MAGIC         },
# MAGIC         "holidays": {
# MAGIC                 "holidays_json_path": The holidays json file path in DBFS. The json file has a dictionary structure with
# MAGIC                                       keys being holiday names (i.e. "easter", "superbowl",..., "christmas") and values
# MAGIC                                       being a list of specific holiday dates over the period of consideration
# MAGIC                                       (i.e. christmas": ["2015-12-25", "2016-12-25", "2017-12-25", ...])
# MAGIC         }
# MAGIC     }
# MAGIC },
...
# MAGIC },
# MAGIC "evaluation": List of evaluators to be included in the experiment and their parameters. Fields:
# MAGIC               - "metric": Class name of the evaluator.
# MAGIC               - other: Other key-word arguments that will be passed to the Evaluator constructor.
# MAGIC [
# MAGIC     {"metric": "WMapeEvaluator"},
# MAGIC     {"metric": "WMapeEvaluator", "express_as_accuracy": True}
# MAGIC ],
...
dbutils.fs.mkdirs(f"dbfs:/{dbfs_directory}")
dbutils.fs.ls("dbfs:/")
...
filename_prefix = f'config_{rf_config["model_params"]["model_name_prefix"]}'
...
# Save Config to DBFS:
manager.save(config=rf_config, path_to_save=filepath_to_save)

# COMMAND ----------

# MAGIC %md
# MAGIC ## Validate results & view finalized config
