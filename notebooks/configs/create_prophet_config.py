...
# MAGIC %md
# MAGIC # Create Configuration File - Prophet Example
# MAGIC 
...
# MAGIC 
# MAGIC The following is an example config that can be used for Prophet. There are 2 approaches for incorporating holidays into Prophet models:
# MAGIC 1. Add a holidays DF as a hyperparameter: This approach creates a holidays dataframe (in a specific pandas DF format, see `/dbfs/tmp/holidays_flna_2016_to_2025.json`) and adds it as a hyperparameter during training.
...
# MAGIC 
# MAGIC If you wish to add any additional features as regressors for Prophet, please add them as `additional_feature_colnames` under the `feature_engineering` section. See the _models_prophet_ notebook under _module_samples_ for an example of how this is done.

# COMMAND ----------

manager = ConfigManager(path_to_config="./json/prophet_config_small.json")
prophet_config = manager.get()
pprint(prophet_config)
...
# MAGIC                        directory for a list of implemented feature engineering operations. For prophet model, no feature
# MAGIC                        engineering operations are needed, but you can input your "additional features" to add to your
# MAGIC                        model as regressors.
# MAGIC {
# MAGIC },
...
filename_prefix = f'config_{prophet_config["model_params"]["model_name_prefix"]}'
...
# Save Config to DBFS:
manager.save(config=prophet_config, path_to_save=filepath_to_save)
